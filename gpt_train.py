...
def main():
    ...
    if len(sys.argv) <= 3:
        ...
        ## rank stuff is for multi-GPU training (possibly with deepspeed)
        rank = None
        ...
        if sys.argv[-1].endswith('.json'):
            json_file = os.path.join(root, sys.argv[-1])

        sys.argv = json.loads(' '.join(read_lines(json_file)))
        if rank is not None:
    ...
    # https://github.com/huggingface/transformers/issues/9785
    ...
    if training_args.do_predict or (data_args.task_name is not None and data_args.train_file is None) or data_args.test_file is not None:
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if data_args.task_name is not None:
            kappa = qwk(preds, p.label_ids).round(4)
            return {"kappa": kappa}
